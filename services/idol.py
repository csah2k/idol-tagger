...
class Service:
    ...
    def suggest_on_text(self, dbname, text, fieldtext='', minscore=60):
        return self.executor.submit(self.suggest_on_text_sync, dbname, text, fieldtext, minscore).result()

    @retry(wait_exponential_multiplier=1000, wait_exponential_max=10000, stop_max_delay=30000)
    def suggest_on_text_sync(self, dbname, text, fieldtext, minscore):    
        response = requests.get(f'{self.dah}/a=SuggestOnText&Responseformat=simplejson&Fieldtext={fieldtext}&DatabaseMatch={dbname}&MinScore={minscore}&Text={text}', verify=False)    
